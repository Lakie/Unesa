...
function toggleDropdown(what, extra, inner) {
	...
	if (RSFormPro.$('#dropdown' + nameid).length == 0) {
		...
		RSFormPro.$(divContainer).click(function (e) {
			e.stopPropagation();
			e.preventDefault();
		});
		...
		if (extra)
			for (var j = 0; j < extra.length; j++) {
				...
				a.onclick = function () {
					dropdownClick(nameid, this, inner);
				};
...
RSFormPro.Post.addField = function () {
	...
	var $inputName = RSFormPro.$('<td><input type="text" name="form_post[name][]" placeholder="' + Joomla.JText._('RSFP_POST_NAME_PLACEHOLDER') + '" class="rs_inp rs_80"></td>');
	var $inputValue = RSFormPro.$('<td><input type="text" name="form_post[value][]" placeholder="' + Joomla.JText._('RSFP_POST_VALUE_PLACEHOLDER') + '" class="rs_inp rs_80"></td>');
