...
class com_rsformInstallerScript
{
	...
	public function update($parent) {
		...
		if (!isset($columns['LoadFormLayoutFramework'])) {
			$db->setQuery("ALTER TABLE `#__rsform_forms` ADD `LoadFormLayoutFramework` TINYINT( 1 ) NOT NULL AFTER `FormLayoutName`");
			$db->execute();
		}
...
.red {
	...
	<div id="installer-right">
		...
		<?php if ($messages['modules']) { ?>
			...
			<?php } ?>
		<?php } ?>
		<h2>Changelog v1.51.10</h2>
		<ul class="version-history">
			<li><span class="version-upgraded">Upg</span> Callback function added to the Date and Time Picker.</li>
			<li><span class="version-fixed">Fix</span> Legacy layouts adjustments ("formRequired" class was missing).</li>
		</ul>
